...
|---|---|
| **Name** | `ohnoo` |
| **Package names** | PyPI: `ohnoo` (or `ohnoo-cli` if taken) · npm: `ohnoo` |
| **Binary** | `ohnoo` |
| **License** | MIT |
...
- **Core CLI:** Python (for the `pip` distribution) — `click` or `typer` for the CLI surface
- **npm wrapper:** thin Node wrapper that shells out to the same Python binary (matches the `SPECTRA` dual-publish pattern already in use) — OR a parallel Node implementation if full JS-native behavior is required for the shell hook on Windows/PowerShell users. Decide in Phase 1 based on hook complexity.
- **Shell hooks:** native zsh/bash/fish scripts, generated by `ohnoo init` and appended to the user's rc file
- **MCP server:** official MCP Python or TypeScript SDK
- **Pattern DB:** plain JSON, versioned, schema-validated in CI
- **Share-card renderer:** headless image generation (e.g. Pillow for a terminal-style PNG) — no browser dependency
- **Packaging:** `pyproject.toml` (Python), `package.json` (npm wrapper), following the same dual pip+npm release process already established for `SPECTRA`
...
- [ ] Decide final PyPI/npm name availability (confirm `ohnoo` is free on both registries with a direct registry query, not just search)
...
- [ ] Dual publish pipeline (PyPI + npm) via CI, mirroring the existing `SPECTRA` release process
...
### 7.2 Home page
- **Hero:** a live terminal demo *is* the hero — not an illustration of one. Show the actual crash → roast → fix sequence from the README GIF, framed as case-file evidence. Install command (`pip install ohnoo` / `npm i -g ohnoo`) sits directly under it, one click to copy.
...
1. Install (`pip install ohnoo` or `npm i -g ohnoo`)
